    ...
    <div class="p-4 overflow-x-auto bg-white rounded-lg shadow-md">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($instalasiFireAlarms as $instalasiFireAlarm)
                <tr>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $loop->iteration }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $instalasiFireAlarm->jobOrderTool->jobOrder->nomor_jo }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $instalasiFireAlarm->jobOrderTool->tool->nama }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $instalasiFireAlarm->jobOrderTool->status }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                        @if ($instalasiFireAlarm->jobOrderTool->status_tool == 'belum')
                        <span class="px-3 py-1 text-sm font-bold text-white rounded-full bg-gradient-to-t from-red-700 to-red-500">
                            {{ ucfirst($instalasiFireAlarm->jobOrderTool->status_tool) }}
                        </span>
                        @else
                    ...
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                        <div class="flex items-center gap-1">
                            ...
                            <button class="flex p-2 transition-all duration-500 rounded-full group item-center">
                                <a href="{{ route('form_kp.ipk.instalasi_fire_alarm.show , $instalasiFireAlarm->id) }}">
                            ...
                            {{-- Update --}}
                            <button class="flex p-2 transition-all duration-500 rounded-full group item-center">
                                <a href="{{ route('form_kp.ipk.instalasi_fire_alarm.edit , $instalasiFireAlarm->id) }}">
