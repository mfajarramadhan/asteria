    ...
    <div class="p-4 overflow-x-auto bg-white rounded-lg shadow-md">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200" id="pesawat-tenaga-produksi-table">
                @forelse ($pesawatTenagaProduksis as $pesawatTenagaProduksi)
                <tr>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $loop->iteration }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $pesawatTenagaProduksi->tanggal_pemeriksaan->format('d-m-Y') }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                        <a href="{{ route('job_orders.show', $pesawatTenagaProduksi->jobOrderTool->jobOrder->id) }}#list-jo" class="hover:underline hover:text-blue-500">
                            {{ $pesawatTenagaProduksi->jobOrderTool->jobOrder->nomor_jo }}
                        </a>
                    </td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $pesawatTenagaProduksi->jobOrderTool->tool->nama }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $pesawatTenagaProduksi->jobOrderTool->jobOrder->nama_perusahaan }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $pesawatTenagaProduksi->jobOrderTool->status }}</td>
                    <td class="px-4 py-3 text-sm text-center text-gray-900 whitespace-nowrap">
                        @if ($pesawatTenagaProduksi->jobOrderTool->status_tool == 'belum')
                                <span class="px-3 py-1 text-sm font-bold text-white rounded-full bg-gradient-to-t from-red-700 to-red-500">
                                    {{ ucfirst($pesawatTenagaProduksi->jobOrderTool->status_tool) }}
                                </span>
                        @else
                    ...
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                        <div class="flex items-center gap-1">
                            ...
                            <a href="{{ route('form_kp.ptp.pesawat_tenaga_produksi.show' , $pesawatTenagaProduksi->id) }}">
                            ...
                            <button class="flex p-2 transition-all duration-500 rounded-full group item-center">
                            <a href="{{ route('form_kp.ptp.pesawat_tenaga_produksi.edit' , $pesawatTenagaProduksi->id) }}">
...
document.getElementById('search').addEventListener('keyup', function () {
    ...
    fetch(`{{ route('form_kp.ptp.pesawat_tenaga_produksi.search') }}?q=${encodeURIComponent(keyword)}`)
        .then(res => res.json())
        .then(data => {
