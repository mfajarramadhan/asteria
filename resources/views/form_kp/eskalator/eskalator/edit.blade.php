<x-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-md">
        <form action="{{ route('form_kp.eskalator.eskalator.update, $formKpEskalator->id) }}" method="POST" class="space-y-4" enctype="multipart/form-data" onsubmit="return confirm('Update data?')">
            @csrf
            @method('PUT')
                ...
                <div class="w-full md:w-[48%]">
                    <div class="relative">
                        ...
                        <input id="datepicker-autohide" name="tanggal_pemeriksaan" placeholder="Tanggal Pemeriksaan"
                            value="{{ old('tanggal_pemeriksaan', Carbon::parse($formKpEskalator->tanggal_pemeriksaan)->format('d-m-Y')) }}"
            ...
            <div>
                ...
                @php $oldFiles = json_decode($formKpEskalator->foto_shell, true); @endphp
            ...
            <div>
                ...
                <input type="text" name="ketidakbulatan" placeholder="Ketidak bulatan" id="ketidakbulatan"
                    ...
                    value="{{ old('ketidakbulatan', $formKpEskalator->ketidakbulatan) }}">
            ...
            <div>
                ...
                <textarea name="catatan" id="catatan" placeholder="Catatan" rows="3"
                    class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm 
                            focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm 
                            @error('catatan') valid:border-red-600 valid:focus:border-red-600 valid:focus:ring-red-200 @enderror">{{ old('catatan', $formKpEskalator->catatan) }}</textarea>
                @error('catatan')
                <div class="text-xs text-red-600">
                    {{ $message }}
                </div>
