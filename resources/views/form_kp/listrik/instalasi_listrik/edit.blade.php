    ...
    <div class="p-4 bg-white rounded-lg shadow-md">
        <form action="{{ route('form_kp.listrik.instalasi_listrik.update', $formKpInstalasiListrik->id) }}" method="POST" class="space-y-4" enctype="multipart/form-data" onsubmit="return confirm('Update data?')">
            ...
            <div>
                ...
                <input type="text" name="pabrik_pembuat" placeholder="Pabrik Pembuat" id="pabrik_pembuat" class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('pabrik_pembuat') valid:border-red-600 valid:focus:border-red-600 valid:focus:ring-red-200 @enderror" value="{{ old('nama_perusahaan', $formKpInstalasiListrik->pabrik_pembuat) }}">
            ...
            <div>
                ...
                {{-- foto lama --}}
                @if($formKpInstalasiListrik->foto_mesin)
                @php $oldFiles = json_decode($formKpInstalasiListrik->foto_mesin, true); @endphp
