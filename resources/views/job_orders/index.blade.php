            ...
            <tbody class="divide-y divide-gray-200" id="job-orders-table">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                        <a href="{{ route('job_orders.show', $jo->id) }}#list-jo" class="hover:underline hover:text-blue-500">
                            {{ $jo->nomor_jo }}
                        </a>
                    ...
                    <td>
                        ...
                        @elseif($jo->status_jo == 'proses')
                            <span class="px-3 py-1 text-sm font-bold text-white rounded-full bg-gradient-to-t from-orange-700 to-orange-500">{{ ucfirst($jo->status_jo) }}</span>
                        @else
                            <span class="px-3 py-1 text-sm font-bold text-white rounded-full bg-gradient-to-t from-green-700 to-green-500">{{ ucfirst($jo->status_jo) }}</span>
                        @endif
                    </td>      
                    {{-- 
                    Total Alat dalam JO
                    ...
                    <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                        <div class="flex items-center gap-1">
                            ...
                            <button class="flex p-2 transition-all duration-500 rounded-full group item-center">
                                <a href="{{ route('job_orders.show', $jo->id) }}">
...
document.getElementById('search').addEventListener('keyup', function () {
    ...
    fetch(`{{ route('job_orders.search.jo') }}?q=${encodeURIComponent(keyword)}`)
        .then(res => res.json())
        .then(data => {
